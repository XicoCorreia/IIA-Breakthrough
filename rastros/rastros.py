...
class EstadoRastros(stateRastros):
    ...
    def __init__(self, to_move, white, blacks):
        self.fullboard = set([(x, y) for x in range(1, 9) for y in range(1, 9)])
        self.terminou = (
            self.ve_se_terminou()
        )  # = 1 se ganhou sul, -1 se ganhou norte, 0 se não terminou

    def moves(self):
        "Legal moves are any square adjacent to white if not in blacks"
        alladjacent = [
            (self.white[0] + a, self.white[1] + b)
            for a in [-1, 0, 1]
            for b in [-1, 0, 1]
        ]
        return [
            p
            for p in alladjacent
...
class Rastros(Game):
    ...
    def result(self, state, move):
        ...
        return EstadoRastros(
            to_move=("N" if state.to_move == "S" else "S"), white=move, blacks=blacks
        )
...
def jogaRastros11com_timeout(jog1, jog2, nsec):
    ...
    while not game.terminal_test(estado):
        ...
        if jogada == None:
            estado.terminou = (
                1 if proxjog == jog2 else -1
            )  # se norte deu timeout ganha sul e vice-versa
            return (lista_jogadas, estado.terminou)
        else:
            estado = game.result(estado, jogada)
            lista_jogadas.append((proxjog.nome, jogada))
...
def jogaRastrosNN(listaJog, listaAdv, nsec=1):
    ...
    for jog in listaJog:
        for adv in listaAdv:
            if jog != adv:
                lista_jogos.append(
                    (jog.nome, adv.nome, jogaRastros11com_timeout(jog, adv, nsec))
                )
    return lista_jogos
...
arlivre = Jogador(
    ...
    lambda game, state: alphabeta_cutoff_search_new(
        state, game, depth_for_all, eval_fn=num_livres
    ),
)
...
basilio = Jogador(
    "Basilio",
    lambda game, state: alphabeta_cutoff_search_new(
        state, game, depth_for_all, eval_fn=f_aval_basico
    ),
)
